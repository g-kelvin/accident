...
<?php 
  ...
  echo "<label style='color:white'>";
       ...
       $qry=mysqli_query($connection,"SELECT id,firstName,lastName, station FROM users where status='pending'AND station='hospital'");
       $no = mysqli_num_rows($qry);
       echo "<center><div id='displaypendingadmins'><h3> The number of pending id $no</h3></div></center>";
